  ...
  namespace Memory {
    ...
    Allocation allocate(control_parameter const control, std::size_t size, std::size_t alignment) noexcept {
      ...
      do {
        ...
        if (NULL != allocation) { kind = Allocation::C_STANDARD; size += alignment * (LAPYS_MAX_BUILTIN_ALIGNMENT != alignment); if (control & Traits::CLEARED) { std::memset(allocation, 0x0, size); } break; }
        ...
        // ...
          ...
          if (NULL != allocation) { kind = Allocation::CPP_STANDARD; size += alignment * (alignmentof(Allocation) != alignment); if (control & Traits::CLEARED) { std::fill(static_cast<byte*>(allocation), static_cast<byte*>(allocation) + size, static_cast<byte>(0x0u)); } break; }
        #endif

        #ifdef __STDCPP_DEFAULT_NEW_ALIGNMENT__
          allocation = ::operator new(size + (alignment * (__STDCPP_DEFAULT_NEW_ALIGNMENT__ != alignment)), std::nothrow);
          if (NULL != allocation) { kind = Allocation::CPP_STANDARD; size += alignment * (__STDCPP_DEFAULT_NEW_ALIGNMENT__ != alignment); if (control & Traits::CLEARED) { std::fill(static_cast<byte*>(allocation), static_cast<byte*>(allocation) + size, static_cast<byte>(0x0u)); } break; }
        #else
          allocation = ::operator new(size + alignment, std::nothrow);
          if (NULL != allocation) { kind = Allocation::CPP_STANDARD; size += alignment; if (control & Traits::CLEARED) { std::fill(static_cast<byte*>(allocation), static_cast<byte*>(allocation) + size, static_cast<byte>(0x0u)); } break; }
        #endif
      } while (false);
