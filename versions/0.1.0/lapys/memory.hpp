...
namespace Lapys {
  namespace Memory {
    namespace Traits {
      ...
      template <class> struct is_heap_allocator;
      template <class> struct is_stack_allocator;
    }
  ...
  namespace Memory {
    ...
    namespace Traits {
      ...
      struct is_allocator final {
        static bool const value = is_heap_allocator<base>::value || is_stack_allocator<base>::value;
      };

      template <class> struct is_heap_allocator final { static bool const value = false; };
      template <typename base> struct is_heap_allocator<HeapAllocator<base> > final { static bool const value = true; };

      template <class> struct is_stack_allocator final { static bool const value = false; };
      template <typename base, std::size_t size> struct is_stack_allocator<StackAllocator<base, size> > final { static bool const value = true; };
    }
